...
# =============================================================================
# AE 8803 PROJECT: DECENTRALIZED SOLUTIONS FOR NAVIGATION
# =============================================================================
...
def simulate(nav_mode):
    ...
    for t in t_range:
        ...
        for i in range(len(X)):
            ...
            pass
        X_vals.append(X[9][0])
        X_vals2.append(X[9][0])
        # print(X[0])
        # X_vals[t] = X[0]
        # print(t,X_vals[0:t],"\n===")
            
        if t%10 == 0:
            # X_vals2.append(X[0][0])
            # X_vals.append(X[0])
            # print(X_vals,"\n===")
...
if __name__ == "__main__":
    # VO
    X_check_vo,goal = simulate("VO")
    
    # HRVO
    X_check_hrvo,goal = simulate("HRVO")
    
    dist_from_goal_vo = []
    dist_from_goal_hrvo = []
    
    for i in range(len(X_check_vo)):
        dist_from_goal_vo.append(abs(X_check_vo[i] - goal[9][0]))
        dist_from_goal_hrvo.append(abs(X_check_hrvo[i] - goal[9][0]))        
        pass
    
    pyplot.close("all")
    pyplot.figure(1)
    pyplot.plot(range(len(dist_from_goal_vo)),dist_from_goal_vo,label="VO")
    pyplot.plot(range(len(dist_from_goal_hrvo)),dist_from_goal_hrvo,label="HRVO")
    pyplot.title(r"Distance of Agent $a_{10}$ from the Goal (x-coordinate)",fontweight="bold")
    pyplot.xlabel("Time instance",fontsize=10)
    pyplot.ylabel("Absolute Value",fontsize=10)

    pyplot.legend(loc="best")
    
    # pyplot.figure(2)
    # pyplot.plot(range(len(vel_from_goal_vo)),vel_from_goal_vo,label="VO")
    # pyplot.plot(range(len(vel_from_goal_hrvo)),vel_from_goal_hrvo,label="HRVO")
    # pyplot.legend(loc="best")
